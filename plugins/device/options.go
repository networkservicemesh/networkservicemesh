...
const (
	// DefaultName - default value of Plugin.Deps.Name
	DefaultName = "device"
)
...
func UseDeps(deps *Deps) Option {
	return func(p *Plugin) {
		...
		d.Name = deps.Name
		d.Log = deps.Log
		d.DevicePluginServer = deps.DevicePluginServer
...
func DefaultDeps() Option {
	return func(p *Plugin) {
		...
		if d.ConfigLoader == nil {
			config.SharedPlugin(config.UseDeps(&config.Deps{Name: d.Name,
				DefaultConfig: &defaultConfig}))
		}
		// Intentially left p.Deps.DevicePluginServer nil
