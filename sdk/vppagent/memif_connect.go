...
func (mc *MemifConnect) Request(ctx context.Context, request *networkservice.NetworkServiceRequest) (*connection.Connection, error) {
	...
	appendMemifInterface(vppAgentConfig, request.GetConnection(), mc.Workspace, true)
...
func (mc *MemifConnect) Close(ctx context.Context, connection *connection.Connection) (*empty.Empty, error) {
	ctx = WithVppAgentConfig(ctx) // Guarantees we will retrieve a non-nil VppAgentConfig from context.Context
	vppAgentConfig := VppAgentConfig(ctx)
	appendMemifInterface(vppAgentConfig, connection, mc.Workspace, true)
...
func appendMemifInterface(rv *configurator.Config, connection *connection.Connection, workspace string, master bool) error {
	...
	if master {
		ipAddresses = append(ipAddresses, connection.GetContext().DstIpAddr)
	} else {
		ipAddresses = append(ipAddresses, connection.GetContext().SrcIpAddr)
	}
