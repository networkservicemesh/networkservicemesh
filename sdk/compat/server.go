...
func (s serverAdapter) Request(ctx context.Context, request *networkservice.NetworkServiceRequest) (*connection.Connection, error) {
	...
	return nil, fmt.Errorf("No NetworkServiceRequestServer available for request.GetMechanismPreferences() Mechanism Cls: %s", class)
}

func (s serverAdapter) Close(ctx context.Context, conn *connection.Connection) (*empty.Empty, error) {
	...
	if conn.GetMechanism().GetCls() == cls.LOCAL && s.localServer != nil {
		...
		return nil, fmt.Errorf("No NetworkServiceServer available for Connection.GetMechanism().GetCls(): %+s", conn.GetMechanism().GetCls())
	}

	return &empty.Empty{}, nil
}

func NewUnifiedServerAdapter(remoteServer remote.NetworkServiceServer, localServer local.NetworkServiceServer) networkservice.NetworkServiceServer {
