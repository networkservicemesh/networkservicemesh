...
func TestParseDefaultGatewayInvalidStartingString(t *testing.T) {
	g := gomega.NewWithT(t)
...
func TestParseProcInvalidFileContent(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	eth0, gw, err := parseProcFile(s)
	g.Expect(err.Error()).To(gomega.Equal("invalid line in proc file"))
	logrus.Printf("Value %v", gw.String())
	g.Expect(eth0).To(gomega.Equal(""))
	g.Expect(gw).To(gomega.BeNil())
}
