...
.PHONY: docker-%-build
docker-%-build::
	@${DOCKERBUILD} --network="host" --build-arg VPP_AGENT=${VPP_AGENT} --build-arg VENDORING="${VENDORING}" --build-arg GOPROXY="${GOPROXY}" --build-arg VERSION=${VERSION} -t ${ORG}/$* -f docker/Dockerfile.$* . && \
	if [ "x${COMMIT}" != "x" ] ; then \
