...
func handleConnection(egress common.EgressInterfaceType, crossConnect *crossconnect.CrossConnect, connect bool, direction uint8) (*crossconnect.CrossConnect, error) {
	...
	if connect {
		...
		err = createRemoteConnection(nsPath, name, ifaceMAC, ifaceIP, egress.SrcIPNet().IP, vxlanIP, cfg.vni, routes, cfg.neighbors)
		if err != nil {
...
func createRemoteConnection(nsPath, ifaceName, ifaceMAC, ifaceIP string, egressIP, remoteIP net.IP, vni int, routes []*connectioncontext.Route, neighbors []*connectioncontext.IpNeighbor) error {
