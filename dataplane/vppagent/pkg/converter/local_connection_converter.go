...
func NewLocalConnectionConverter(c *local.Connection, name string, ipAddressKey string) *LocalConnectionConverter {
	return &LocalConnectionConverter{
		Connection:   c,
		name:         name,
...
func (c *LocalConnectionConverter) ToDataRequest(rv *rpc.DataRequest) (*rpc.DataRequest, error) {
	if c == nil {
	...
	logrus.Infof("m.GetParameters()[%s]: %s", local.InterfaceNameKey, m.GetParameters()[local.InterfaceNameKey])
	...
	rv.LinuxInterfaces = append(rv.LinuxInterfaces, &linux_interfaces.LinuxInterfaces_Interface{
		...
		Enabled:     true,
		Description: m.GetParameters()[local.InterfaceDescriptionKey],
		IpAddresses: []string{c.GetContext()[c.ipAddressKey]}, // TODO - this is wrong... need to use Dst or Src key selectively
		HostIfName:  m.GetParameters()[local.InterfaceNameKey],
