...
docker build --network="host" -t "${ORG}" -f- "${BIN_DIR}" <<EOF
    ...
    RUN mkdir /tmp/vpp/vppagent-nsc/; echo 'Endpoint: "0.0.0.0:9113"' > /tmp/vpp/vppagent-nsc/grpc.conf;echo "disabled: true" > /opt/vpp-agent/dev/telemetry.conf
    RUN mkdir /tmp/vpp/vppagent-icmp-responder-nse/; echo 'Endpoint: "0.0.0.0:9112"' > /tmp/vpp/vppagent-icmp-responder-nse/grpc.conf;echo "disabled: true" > /opt/vpp-agent/dev/telemetry.conf
    RUN mkdir /tmp/vpp/vppagent-firewall-nse/; echo 'Endpoint: "0.0.0.0:9112"' > /tmp/vpp/vppagent-firewall-nse/grpc.conf;echo "disabled: true" > /opt/vpp-agent/dev/telemetry.conf
EOF
