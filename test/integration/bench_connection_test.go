...
func TestOneTimeCommenctingMemif(t *testing.T) {
	RegisterTestingT(t)
	if testing.Short() {
		t.Skip("Skip, please run without -short")
		return
	}
	testOneTimeConnecting(1, nsmd_test_utils.DeployVppAgentNSC, nsmd_test_utils.DeployVppAgentICMP, nsmd_test_utils.IsVppAgentNsePinged)
}
func TestOneTimeCommencting(t *testing.T) {
	RegisterTestingT(t)
	if testing.Short() {
		t.Skip("Skip, please run without -short")
		return
	}
	testOneTimeConnecting(2, nsmd_test_utils.DeployNSC, nsmd_test_utils.DeployICMP, nsmd_test_utils.IsNsePinged)
}
...
func TestOneToOne(t *testing.T) {
	...
	testOneToOne(2, nsmd_test_utils.DeployNSC, nsmd_test_utils.DeployICMP, nsmd_test_utils.IsNsePinged)
}

func TestOneToOneMemif(t *testing.T) {
	RegisterTestingT(t)
	if testing.Short() {
		t.Skip("Skip, please run without -short")
		return
	}
	testOneToOne(1, nsmd_test_utils.DeployVppAgentNSC, nsmd_test_utils.DeployVppAgentICMP, nsmd_test_utils.IsVppAgentNsePinged)
}

func testOneTimeConnecting(nodeCount int, nscDeploy, icmpDeploy nsmd_test_utils.PodSupplier, nsePing nsmd_test_utils.NsePinger) {
...
func testOneToOne(nodeCount int, nscDeploy, icmpDeploy nsmd_test_utils.PodSupplier, pingNse nsmd_test_utils.NsePinger) {
	k8s, err := kube_testing.NewK8s()
	defer k8s.Cleanup()
