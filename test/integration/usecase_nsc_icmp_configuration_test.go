...
func testNSCAndICMP(t *testing.T, nodesCount int, useWebhook bool, disableVHost bool) {
	...
	if useWebhook {
		awc, awDeployment, awService := kubetest.DeployAdmissionWebhook(k8s, "nsm-admission-webhook", GetContainerRepo()+"/admission-webhook", k8s.GetK8sNamespace())
	...
	if len(failures) > 0 {
		...
		kubetest.PrintLogs(k8s, nodes_setup)
		nscInfo.PrintLogs()
