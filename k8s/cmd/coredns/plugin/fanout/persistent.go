...
//TODO: create interface for transport and hide all private methods.
	...
	for {
		...
		case proto := <-t.dial:
			...
			if stack := t.connections[proto]; len(stack) > 0 {
				...
				// Transport methods anymore. So, it's safe to close them in makeRecordA separate goroutine
		...
		case conn := <-t.yield:
			...
			t.connections["tcp-tls"] = append(t.connections["tcp-tls"], &persistConn{conn, time.Now()})
