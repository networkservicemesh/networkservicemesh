...
type NetworkServiceManagerLister interface {
	...
	List(selector labels.Selector) (ret []*v1.NetworkServiceManager, err error)
...
// List lists all NetworkServiceManagers in the indexer.
func (s *networkServiceManagerLister) List(selector labels.Selector) (ret []*v1.NetworkServiceManager, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.NetworkServiceManager))
	})
	return ret, err
}
...
type NetworkServiceManagerNamespaceLister interface {
	...
	Get(name string) (*v1.NetworkServiceManager, error)
...
// List lists all NetworkServiceManagers in the indexer for a given namespace.
func (s networkServiceManagerNamespaceLister) List(selector labels.Selector) (ret []*v1.NetworkServiceManager, err error) {
...
// Get retrieves the NetworkServiceManager from the indexer for a given namespace and name.
func (s networkServiceManagerNamespaceLister) Get(name string) (*v1.NetworkServiceManager, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("networkservicemanager"), name)
	}
	return obj.(*v1.NetworkServiceManager), nil
}
