...
func TestPrefixServiceUpdateConnection(t *testing.T) {
	...
	wrapper, err := service.UpdateConnection(context.TODO(), conn)

	g.Expect(err).To(BeNil())
	g.Expect(wrapper.GetContext().GetIpContext().GetExcludedPrefixes()).To(Equal([]string{"10.10.1.0/24", "10.32.1.0/16"}))
}
...
func TestPrefixServiceValidateConnection(t *testing.T) {
	...
	result, err = service.ValidateConnection(context.TODO(), conn)
