...
func dial(ctx context.Context, unixSocketPath string, timeout time.Duration) (*grpc.ClientConn, error) {
...
func (nsm *NetworkServiceManagerDevicePlugin) Start() error {
	...
	conn, err := dial(context.TODO(), nsm.socket, 5*time.Second)
	if err != nil {
		return err
	}
	conn.Close()
...
func (nsm *NetworkServiceManagerDevicePlugin) Register(kubeletEndpoint, resourceName string) error {
	conn, err := dial(context.TODO(), kubeletEndpoint, 5*time.Second)
	if err != nil {
		return err
	}
	defer conn.Close()
