...
GKE_GROUP=$(gcloud container clusters describe "$GKE_CLUSTER_NAME" --project="$GKE_PROJECT_ID" --zone "$GKE_CLUSTER_ZONE" --format=text | grep "^instanceGroupUrls\[0\]" | sed 's/^instanceGroupUrls\[0\]:.*\///g')
